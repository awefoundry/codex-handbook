...
|---|---|---|---|---|
| 任务开始前 | 当前仓库/分支、模型、推理强度、profile、sandbox 和网络开关可被一次检查 | 作者在 Codex Desktop 的脱敏截图 | 待作者截图 | `07-01-start-checklist.png`；不使用登录态或真实敏感仓库 |
| 执行中 | 审批提示展示完整命令、目录、网络行为和批准范围 | 作者在临时仓库触发的无害审批提示 | 待作者截图 | `07-02-approval-check.png`；只停留在提示，不批准 |
| 提交或交付前 | diff、测试结果、计划外文件和敏感输出已被检查 | 作者在版本控制面板的脱敏截图 | 待作者截图 | `07-03-final-diff-check.png`；保持未提交，不推送或部署 |
...
- 未下载任何视频、缩略图或社区图片；`manifest.tsv` 保持空候选状态，避免把无法证明操作的素材混入正文。
