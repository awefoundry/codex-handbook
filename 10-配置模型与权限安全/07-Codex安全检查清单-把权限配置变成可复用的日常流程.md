...
- [ ] `07-01-start-checklist.png`：Codex Desktop 临时仓库 → 任务输入框下方权限控件 → 显示模型、profile、沙箱、网络和分支状态；隐藏完整路径、账号、仓库私有名称和敏感文件；截图后关闭菜单，不批准未知命令。
- [ ] `07-02-approval-check.png`：临时仓库中触发无害审批提示 → 展开完整命令、目录和网络说明 → 显示一次性/会话级批准范围；隐藏用户目录和项目名；只截图并取消，不要批准或执行。
- [ ] `07-03-final-diff-check.png`：版本控制面板 → 显示提交前 diff、最小测试结果和待处理风险；隐藏项目机密、私人路径、远程仓库地址和 token；不要点击提交、推送、部署或发布。
...
- 官方文档页面发布时间按 GMT 显示为 2026-08-27，本次研究按美国东部时间 2026-08-26 记录；`manifest.tsv` 当前没有保存任何候选图片。
