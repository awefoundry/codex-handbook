...
- `generated-zimacode/task_gptimage_1787905042581_c0277109-1.png` -> `正文配图/图三.png` -> `发布素材/正文配图/图三.png`：配置来源与覆盖优先级解释图，插入“生效优先级”说明之后。
- `generated-zimacode/task_gptimage_1787906470671_d1baa42b-1.png` -> `正文配图/图四.png` -> `发布素材/正文配图/图四.png`：配置不生效排查流程图，插入“配置不生效的排查顺序”列表之后。

两张正文图均使用 imagegzh 一号样式：保留原始文字与布局，增加约 4 px 白色内边距、透明圆角、`#d9d9d9` 细边框和轻阴影；原始截图仍保留在 `manual/`，未覆盖。

图三、图四为 ZimaCode GPT Image 2 生成的解释型插图，使用固定 `https://gen.bunjs.dev`、`engine=web` 和 `gpt-image-2-text-to-image`。两图均为非官方界面示意，不承载新的产品事实；原图、提示词和任务记录保存在 `generated-zimacode/`。
...
- 正文图片引用、发布素材目录和备份映射：通过；正文配图按 `图一.png` 至 `图四.png` 连续编号，`封面.png` 单独命名且不计入正文图号。
...
- 任务 `task_gptimage_1787905042581_c0277109`：配置优先级分层图，1672 × 941；标签 `CLI`、`PROJECT`、`PROFILE`、`USER`、`DEFAULT` 清晰，未见敏感信息。
- 任务 `task_gptimage_1787906470671_d1baa42b`：配置排查流程图，1672 × 941；7 个检查点和回退分支清晰，未见敏感信息。
- 两图均已复制到 `发布素材/正文配图/图三.png`、`图四.png`，并在正文对应章节后各保留独立图注；生成图为概念解释，不冒充 Codex 官方界面。
