...
![Codex 安装登录排障教程封面](../图片素材/01-安装与首次使用/13-安装登录常见问题/00-封面.png)
...
这篇文章把前面几篇安装教程中出现过的问题集中到一起，并补充 OpenAI 官方文档里的 Windows 排障项。文中会明确区分实测画面、官方建议和公开 Issue，方便你判断每条信息来自哪里。
...
![PowerShell 中的 Codex CLI 版本与命令路径检查结果，用户名已脱敏](../图片素材/01-安装与首次使用/13-安装登录常见问题/01-检查CLI版本与路径.png)

`codex --version` 用来确认版本，`Get-Command codex` 会列出当前执行的程序路径。图中采集时为 CLI 0.146.1；本文当前测试版本见文首。

## Microsoft Store 初始化失败，App 装不上

下面这个报错来自前面的 Windows 安装实操。Microsoft Store 打开后没有进入应用详情页，页面直接提示“Microsoft Store 初始化失败”。

![Microsoft Store 提示初始化失败并要求刷新页面](../图片素材/01-安装与首次使用/13-安装登录常见问题/02-Microsoft-Store初始化失败.png)

先点击“刷新页面”，再完全退出 Microsoft Store 后重开。如果仍然失败，检查系统时间、地区和当前网络，尤其是代理是否让 Store 处于半连接状态。

OpenAI 当前的 Windows App 文档还提供了 `winget` 安装入口。用普通 PowerShell 执行以下命令。
...
![Codex CLI 返回 Logged in using ChatGPT 的登录状态](../图片素材/01-安装与首次使用/13-安装登录常见问题/03-CLI登录状态.png)

`Logged in using ChatGPT` 表示当前这套 CLI 已经取得 ChatGPT 登录状态。
...
![Codex CLI 已启动但模型仍停在 loading 状态](../图片素材/01-安装与首次使用/13-安装登录常见问题/04-CLI模型加载中.png)

图中的 CLI 0.146.1 已经启动，模型仍停在加载状态。此时应先检查认证和联网，不必马上重装。
...
![VS Code 扩展市场中由 OpenAI 发布的 Codex 扩展](../图片素材/01-安装与首次使用/13-安装登录常见问题/05-VS-Code官方Codex扩展.png)
...
![VS Code 中 Codex 扩展显示 ChatGPT 和 API Key 登录入口](../图片素材/01-安装与首次使用/13-安装登录常见问题/06-VS-Code登录入口.png)

扩展安装并正常加载后，右侧会显示 ChatGPT 和 API Key 两种登录入口。
...
![VS Code 中的 Codex 面板没有加载出登录入口，只显示空白区域和加载图标](../图片素材/01-安装与首次使用/13-安装登录常见问题/07-VS-Code面板空白.png)

图片来自 [OpenAI Codex Issue #28280](https://github.com/openai/codex/issues/28280)。

![VS Code Insiders 弹出 The window is not responding 提示](../图片素材/01-安装与首次使用/13-安装登录常见问题/08-VS-Code窗口无响应.png)
...
![VS Code 登录成功后显示 Codex 对话输入框](../图片素材/01-安装与首次使用/13-安装登录常见问题/09-VS-Code登录成功.png)
...
- [JetBrains 中使用 Codex](11-JetBrains中使用Codex.md)
- [Codex 更新与卸载](12-Codex更新与卸载.md)

## 官方参考

- [ChatGPT 桌面 App for Windows](https://learn.chatgpt.com/docs/windows/windows-app)
