...
- [x] 文件名：`01-skill-list-result.png`
...
## 2. 安装 img-convert Skill（已完成）

- [x] 文件名：`02-skill-install-success.png`
- 起始位置：Codex Desktop 当前任务 → 终端面板。
- 操作：运行文章中的 `npx -y skills add ... --skill img-convert --agent codex --global --yes` 命令。
...
- [x] 文件名：`03-cli-help-and-version.png`
- 起始位置：Codex Desktop 当前任务 → 终端面板。
...
- [x] 文件名：`04-dry-run-file-list.png`
...
- [x] 文件名：`05-batch-convert-json-result.png`
...
- [x] 文件名：`06-original-images-collage.png`
- 起始位置：`D:\images\original` 中的三张公开测试图片。
- 操作：按一张横幅、两张并排图片的布局直接拼接原始像素，并添加格式、尺寸与转换结果标签。
- 预期画面：桥梁 JPG、GitHub PNG 和山谷 WebP 的真实画面清晰可见，标签与 CLI JSON 数据一致。
- 脱敏：素材均来自公开 URL，不使用客户图片或私人照片。
- 停止位置：导出 1280×900 PNG 后检查画面、文字和边缘，不修改原文件。
