...
> 测试环境：Windows build 26100；Codex CLI 0.147.0；Git 2.47.0.windows.2；2026-08-30 核验。

## 先问对问题

代码审查的第一步不是让 Codex 总结“这个提交做了什么”，而是先把比较范围说清楚。当前工作区、暂存区、某个提交和远程分支，可能是四个不同的现场。
...
把它们连起来，才是一条可复核的证据链：`status` 定位现场，`diff` 读取行为，`log` 补历史，`blame` 在有疑问的行上继续追溯。
...
怀疑某一行的默认值、异常处理或兼容逻辑时，再使用：

    git blame -L 20,45 -- src/api/login.ts

输出中的短提交号、作者和日期表示该行最后一次被某个提交写入。下一步通常是：

    git show <commit> -- src/api/login.ts
    git log --follow --oneline -- src/api/login.ts

如果代码经历过格式化、文件移动或复制，可以尝试：
...
如果只想追踪一个可疑常量，可以缩小范围：
    ...
    区分：这次提交实际改变的内容、提交信息声称的目的、仍然未知的兼容性影响。
...
配套的 `git-baseline-demo` 位于第 01 篇文章的备份目录，是一个可丢弃的离线仓库，里面故意同时放了已暂存、未暂存和未跟踪改动。请在教程仓库根目录运行：
...
> 本节用于写作阶段选图，发布前将采用的素材移动到正文段落并重新编号。

### 官方与可核验操作界面

- 暂无直接覆盖本地 `status`、`diff`、`log`、`blame` 四步的外部操作截图。
...
- 已核验 Git status、diff、log 与 blame 官方文档；平台素材工作区：`./git-xie-zuo-yu-dai-ma-shen-cha-图片备份-20260830-0000/`。
