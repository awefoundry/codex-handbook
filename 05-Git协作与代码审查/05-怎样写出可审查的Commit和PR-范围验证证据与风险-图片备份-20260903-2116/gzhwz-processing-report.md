...
- 正文配图数量：5 张；二维码是结尾 CTA 素材，不计入正文图号。
...
- 按正文首次出现顺序加入提交范围、交互式暂存、PR 描述、diff 设置、审查与回滚五个视觉节点。
...
| 图五 | `hiapi-source/03-hiapi-review-loop.png` | `正文配图/图五.png` | HiAPI 生成概念插图 | 1280×720，16:9 | 已用 `view_image` 检查。表达提交、验证、审查、风险和回滚流程；无可读账号、路径、Token、Logo、二维码或水印。 |
...
- 图一任务：`tk-hiapi-01M1NRN8JWYMG739PQWCPX3AE1`。
- 图三任务：`tk-hiapi-01M1NRNBFTXP2BW4D4MVVG674Z`。
- 图五任务：`tk-hiapi-01M1NRNEFTWZM7JY8PRS9YYEVQ`。
- 三个任务最终状态均为 `success`。生成原图未覆盖，发布图由本地 Pillow 处理。
...
- 正文图片引用按首次出现顺序为 `正文配图/图一.png`、`正文配图/图二.png`、`正文配图/图三.png`、`正文配图/图四.png`、`正文配图/图五.png`，与文件逐项对应。
- `正文配图/` 只包含五张正文图，没有原始截图和二维码。
- 发布正文未出现“用户提供”“用户素材”“用户截图”“投稿截图”“作者截图”、本机绝对路径、内部处理文件名或任务 ID。
- 生成概念图均为 16:9，比例误差为 0；图二为 1129×249 证据裁切图，主体和文字可读；原件保留在备份目录。
