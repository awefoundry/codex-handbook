...
正文处理：已按 gzhwz 要求将原“正文计划”改写为教程正文，保留官方资料、截图事实和未完成实验边界；human-writing 检查脚本未发现翻案句、三段式排比或模型化连接词，技术来源行的链接标点提示已保留。

发布正文清理：已移除状态、实操计划、完成标准、素材备用区和检索记录等内部工作内容；正文只保留读者可见的教程段落、3 张正文配图和最终检查清单。
...
- `manual/` 下的 `图一.png` 至 `图五.png` 为未引用的备选素材，不计入正文编号；作者只需按 `manual-steps.md` 补 1 张审批截图。
...
- 上线阻塞项：危险动作的可丢弃测试记录、动作风险对照表、拒绝审批后的替代方案结论，以及可核验的网站 canonical URL。必需的 1 张作者实拍审批截图已收到并归档，正文解释和检查顺序已完成。

## 上线闸门

- [x] 正文图片 6 张均有处理副本、来源记录和可解析引用。
- [x] 官方资料来源与 AI 示意图已在图注中区分。
- [x] 1 张作者实拍审批截图已归档；其余结果用文字记录和已有来源支撑。
- [ ] 可丢弃测试的完整文字记录、动作风险对照表和拒绝后的替代方案结论完成。
- [x] 4 项正文完成标准完成并复核。
